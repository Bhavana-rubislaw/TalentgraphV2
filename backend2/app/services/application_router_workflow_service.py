...
class ApplicationRouterWorkflowService:
    ...
    def apply_to_job(*, data, request, current_user: dict, session: Session):
        ...
        logger.info(
            f"[APPLICATION_ROUTER_WORKFLOW] apply_to_job user_id={current_user.get('user_id')} request_id={request_id}"
        )
    ...
    def update_application_status(*, application_id: int, data, request, current_user: dict, session: Session):
        request_id = getattr(request.state, "request_id", None)
        logger.info(
            f"[APPLICATION_ROUTER_WORKFLOW] update_application_status application_id={application_id} user_id={current_user.get('user_id')} request_id={request_id}"
        )
        user = UserContextService.get_user_by_email_or_404(session, current_user["email"])
    ...
    def update_application_review(*, application_id: int, data, request, current_user: dict, session: Session):
        request_id = getattr(request.state, "request_id", None)
        logger.info(
            f"[APPLICATION_ROUTER_WORKFLOW] update_application_review application_id={application_id} user_id={current_user.get('user_id')} request_id={request_id}"
        )
        user = UserContextService.get_user_by_email_or_404(session, current_user["email"])
        if user.role not in ("recruiter", "hr"):
            raise HTTPException(status_code=403, detail="Recruiters and HR only")
    ...
    def withdraw_application(*, application_id: int, request, current_user: dict, session: Session):
        request_id = getattr(request.state, "request_id", None)
        logger.info(
            f"[APPLICATION_ROUTER_WORKFLOW] withdraw_application application_id={application_id} user_id={current_user.get('user_id')} request_id={request_id}"
        )
        user = UserContextService.get_user_by_email_or_404(session, current_user["email"])
