...
async def google_authorize(
    ...
    auth_url = (
        ...
        f"&prompt=consent"
        f"&state={current_user['user_id']}"  # Include user ID in state for security
    )
...
async def list_calendar_accounts(
    ...
    accounts = session.exec(
        select(CalendarAccount).where(CalendarAccount.user_id == current_user["user_id"])
    ).all()
...
async def list_video_provider_accounts(
    ...
    accounts = session.exec(
        select(VideoProviderAccount).where(VideoProviderAccount.user_id == current_user["user_id"])
    ).all()
