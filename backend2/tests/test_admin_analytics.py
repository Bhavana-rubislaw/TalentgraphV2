...
class TestAdminAnalyticsFunnel:
    def test_funnel_conversion_percentages(self, client, db_session):
        from app.security import hash_password, create_access_token
        from app.models import Swipe, Meeting, MeetingStatus
        ...
        # get_admin_analytics computes the funnel from live tables
        # (Swipe/Application/Meeting), not AnalyticsRollupDaily - the
        # rollup table was retired in favor of direct queries (see git
        # history: "Rebuilt analytics queries to use live production
        # tables instead of empty rollups"). Seed the tables the
        # endpoint actually reads instead of the unused rollup.
        recent = datetime.utcnow() - timedelta(days=5)

        db_session.add_all([
            Swipe(
                candidate_id=1, company_id=1, job_profile_id=1, job_posting_id=1,
                action="like", action_by="candidate", created_at=recent,
            )
            for _ in range(1000)
        ])
        db_session.add_all([
            Application(
                candidate_id=1, job_posting_id=1, job_profile_id=1,
                status="hired" if i < 20 else "applied",
                applied_at=recent,
                last_status_updated_at=recent if i < 20 else None,
            )
            for i in range(250)
        ])
        db_session.add_all([
            Meeting(
                ...
                scheduled_start=recent,
                scheduled_end=recent + timedelta(hours=1),
...
class TestAdminAnalyticsTopCompanies:
    def test_top_companies_sorted_by_activity_score(self, client, db_session):
        ...
        # top_companies is computed by joining JobPosting -> Application /
        # Meeting on company_id (see get_admin_analytics in admin.py), not
        # from AnalyticsRollupDaily - same retired data source as the
        # funnel test above. Seed real JobPosting/Application/Meeting rows
        # per company instead.
        for i, (apps, interviews, hires) in enumerate([(100, 50, 10), (200, 80, 20)]):
            user = User(
                email=f"topco_user{i}@example.com",
            ...
            db_session.add_all([
                Application(
                    candidate_id=1, job_posting_id=job_posting.id, job_profile_id=1,
                    status="hired" if a < hires else "applied",
                    applied_at=recent,
                    last_status_updated_at=recent if a < hires else None,
                )
                for a in range(apps)
            ])
            db_session.add_all([
                Meeting(
                    title=f"Interview {i}-{m}",
                    organizer_user_id=user.id,
